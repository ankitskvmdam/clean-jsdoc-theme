...
- **Islands** — `Sidebar`, `MobileNav`, `TOC`, `TocPopover`, `CtrlK`, `CodeTabs`, `CopyBtn`, `CopyPageButton`, `ThemeToggle`, `Settings`, `LanguageSwitcher`, `CodeViewer`, `Embed`, `Tabs`. Each renders meaningful initial HTML and progressively enhances after hydration (keyboard handling, focus trap, scroll-spy, drawer state, clipboard, theme/setting persistence, locale navigation, a sandboxed iframe, a CDN-loaded Monaco source viewer).
- **SSR-only render helpers** — `Steps`/`Step` (numbered stepper), `PageNav` (prev/next pager). These emit static markup with no island of their own.
- **Primitives** — shadcn-style `Button` (`buttonVariants`), `ButtonGroup` (segmented/split buttons), `DropdownMenu` (compound root/trigger/content/item/…), `Dialog` (+ `DialogHeader`/`DialogTitle`/`DialogBody`/`DialogFooter`), `CodeBlock`, and the `cn` class-merge helper.
- **`defaultMdxComponents`** — element map for `@mdx-js/mdx`: headings with hover anchor links, external-link `rel`/`target` heuristics, `<CopyBtn>`-wrapped code blocks, table/list/quote primitives, and the `<Callout>` / `<Steps>` / `<Tabs>` / `<Embed>` elements.
- **`ISLAND_REGISTRY`** — `Record<IslandName, ComponentType>` keyed by the `IslandName` union from `@clean-jsdoc-theme/utils`. The fourteen keys: `sidebar`, `mobile-nav`, `toc`, `toc-mobile`, `cmdk`, `code-tabs`, `code-viewer`, `embed`, `copy-btn`, `copy-page`, `theme-toggle`, `settings`, `language-switcher`, `tabs`.
