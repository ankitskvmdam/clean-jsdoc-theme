...
| `llmsTxt` | boolean | object | Emit an llmstxt.org index: `llms.txt` at the output root (one `##` section per sidebar group; each entry links the page's companion `.md`, not its HTML) plus `llms-full.txt` (all bodies concatenated). `true` = `{ full: true, api: true }`. `full: false` skips `llms-full.txt`; `api: 'index'` lists API pages without descriptions and omits their bodies; `api: false` drops API pages entirely. **Requires `siteUrl`** (links must be absolute) — without it the build warns and emits nothing. Source-file pages are never listed; each locale gets its own file. |
...
| `meta` | `Array<{ [attr]: value }>` | Site-wide custom `<meta>` tags. Each object's key/value pairs become one `<meta>` (`{ name, content }`, `{ property, content }`, …). Theme defaults (charset/viewport/`generator` version stamp/auto description) emit first; an author entry sharing a `name`/`property`/`http-equiv`/`charset` replaces the default (no dupes). Values escaped; invalid attr names dropped. |
| `hashCustomAssets` | boolean | Content-hash custom-asset filenames for cache-busting. Default `true`. |
| `scrollbar` | `"styled"` \| `"visible"` \| `"native"` | Scrollbar presentation. `styled` (default): thin themed overlay bar, hidden at rest, painted while scrolling/hovering. `visible`: same themed bar, always shown. `native`: no styling — the browser's own scrollbar (issue #281). Works under **both** JSDoc and TypeDoc. |
